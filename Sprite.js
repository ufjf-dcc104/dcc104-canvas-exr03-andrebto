...
Sprite.prototype.colidiuCom = function (alvo) {
  var onxTop = alvo.x >= this.x && alvo.x <= (this.x + this.width);
  var onyTop = alvo.y >= this.y && alvo.y <= (this.y + this.height);
  var onxBottom = (alvo.x + alvo.width) >= this.x && (alvo.x + + alvo.width) <= (this.x + this.width);
  var onyBottom = (alvo.y + alvo.height) >= this.y && (alvo.y + alvo.height) <= (this.y + this.height);
  return (onxTop && onyTop) || (onxBottom && onyBottom);
};
